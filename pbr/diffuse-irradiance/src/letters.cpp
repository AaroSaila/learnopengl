...
constexpr std::array B_vertices {
    0.0f, -0.25f,
    0.0f, 0.25f,
    0.125f, 0.25f / 2.0f,
    0.0f, 0.0f,
...
constexpr std::size_t B_vert_count { B_vertices.size() / 2 };
...
constexpr std::size_t G_vert_count { G_vertices.size() / 2 };
...
void draw_letter(
    ...
    case Letters::B:
        if (B_vao == 0) {
            B_vao = letter_vao_make(B_vertices.data(), B_vertices.size());
        }
        vao = B_vao;
        vert_count = B_vert_count;
        break;

    case Letters::G:
        if (G_vao == 0) {
            G_vao = letter_vao_make(G_vertices.data(), G_vertices.size());
        }
        vao = G_vao;
