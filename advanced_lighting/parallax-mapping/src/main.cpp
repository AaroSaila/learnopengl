...
static bool parallax_map_disabled { false };
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    case GLFW_KEY_N:
        switch (action) {
        case GLFW_PRESS:
            normal_map_disabled = parallax_map_disabled ? !normal_map_disabled : false;
            break;
        }
        break;
    ...
    case GLFW_KEY_P:
        switch (action) {
        case GLFW_PRESS:
            parallax_map_disabled = normal_map_disabled ? true : !parallax_map_disabled;
            break;
        }
        break;
    }
...
int main(const int argc, const char** argv) {
    ...
    const unsigned int brick_texture {
        texture_load(textures_path / "bricks2.jpg", GL_SRGB, GL_RGB)
    };
    const unsigned int brick_normal_map {
        texture_load(textures_path / "normal-maps" / "bricks2_normal.jpg", GL_RGB, GL_RGB)
    };
    const unsigned int brick_depth_map {
        texture_load(textures_path / "height-maps" / "bricks2_disp.jpg", GL_RGB, GL_RGB)
    };

    const unsigned int toy_box_texture {
        texture_load(textures_path / "toy_box_diffuse.png", GL_SRGB, GL_RGB)
    };
    const unsigned int toy_box_normal_map {
        texture_load(textures_path / "normal-maps" / "toy_box_normal.png", GL_RGB, GL_RGB)
    };
    const unsigned int toy_box_depth_map {
        texture_load(textures_path / "height-maps" / "toy_box_disp.png", GL_RGBA, GL_RGBA)
    };
    ...
    wall_model = glm::translate(wall_model, glm::vec3 { -1.0f, 0.0f, 0.0f });

    glm::mat4 toy_box_model { 1.0f };
    toy_box_model = glm::translate(toy_box_model, glm::vec3 { 1.0f, 0.0f, 0.0f });
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        shader.set_bool("parallax_map_disabled", parallax_map_disabled);
        ...
        glBindVertexArray(square_vao);
            shader.set_float("shininess", 33.0f);
            ...
            glActiveTexture(GL_TEXTURE2);
                glBindTexture(GL_TEXTURE_2D, brick_depth_map);
            glActiveTexture(GL_TEXTURE0);

            glDrawArrays(GL_TRIANGLES, 0, square_vert_count);

            shader.set_mat4("model", toy_box_model);

            glActiveTexture(GL_TEXTURE0);
                glBindTexture(GL_TEXTURE_2D, toy_box_texture);
            glActiveTexture(GL_TEXTURE1);
                glBindTexture(GL_TEXTURE_2D, toy_box_normal_map);
            glActiveTexture(GL_TEXTURE2);
                glBindTexture(GL_TEXTURE_2D, toy_box_depth_map);
            glActiveTexture(GL_TEXTURE0);
        ...
        draw_letters_in_corner_red_green(
            (const Letters[]) { Letters::N, Letters::P },
            2,
            (const glm::vec3[]) { glm::vec3 { 0.0f } },
            1,
            (const bool[]) {
                !normal_map_disabled,
                !parallax_map_disabled },
            Corners::TOPLEFT,
            glm::vec3 { 0.5f },
            letter_shader);
        glEnable(GL_DEPTH_TEST);
