...
constexpr std::size_t P_vert_count { N_vertices.size() / 2 };
...
unsigned int letter_vao_make(const float* vertices, const std::size_t vertices_size) {
    unsigned int vao { };
...
void draw_letter(
    ...
    if (background_color != glm::vec3 { 0.0f }) {
        ...
        glBindVertexArray(background_vao);
            glDrawArrays(GL_TRIANGLE_FAN, 0, background_vert_count);
        glBindVertexArray(0);
        // clang-format on
    }
