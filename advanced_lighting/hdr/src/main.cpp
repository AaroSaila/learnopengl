...
int main(const int argc, const char** argv) {
    ...
    unsigned int hdr_color_buf_texture { };
    glGenTextures(1, &hdr_color_buf_texture);
    glBindTexture(GL_TEXTURE_2D, hdr_color_buf_texture);
    glTexImage2D(
        GL_TEXTURE_2D,
        0,
        GL_RGBA16F,
    ...
    Shader hdr_shader {
        shaders_path / "hdr.vert",
        shaders_path / "hdr.frag",
    };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glBindFramebuffer(GL_FRAMEBUFFER, hdr_fbo);
            ...
            glBindVertexArray(cube_vao);
                ...
                static_assert(light_positions.size() == 4);
                static_assert(light_positions.size() == light_colors.size());
                for (std::size_t i { 0 }; i < light_positions.size(); i++) {
                    shader.set_vec3("point_lights["+std::to_string(i)+"].pos", light_positions[i]);
                    shader.set_vec3("point_lights["+std::to_string(i)+"].color", light_colors[i]);
                }
        ...
        glBindVertexArray(square_vao);
            ...
            hdr_shader.use();
            hdr_shader.set_bool("hdr_disabled", hdr_disabled);
